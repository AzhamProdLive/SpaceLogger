...
module.exports = {
  ...
  func: async interaction => {
    ...
    sa
      .post(`${process.env.PASTE_CREATE_ENDPOINT.endsWith("/") ? process.env.PASTE_CREATE_ENDPOINT.slice(0, -1) : process.env.PASTE_CREATE_ENDPOINT}/documents`)
      ...
      .end((err, res) => {
        if (!err && res.statusCode === 200 && res.body.key) {
          interaction.editOriginalMessage({
            embeds: [{
              title: 'Success',
              description: `Archived ${fetchedMessages.length} messages: ${process.env.PASTE_CREATE_ENDPOINT.endsWith("/") ? process.env.PASTE_CREATE_ENDPOINT.slice(0, -1) : process.env.PASTE_CREATE_ENDPOINT}/${res.body.key}.txt`,
              thumbnail: {
                url: interaction.member.user.dynamicAvatarURL(null, 64)
              },
